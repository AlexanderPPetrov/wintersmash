...
var sounds = {
    ...
    smashing: "sounds/smashing.mp3",
    splendid: "sounds/splendid.mp3",
    marvelous: "sounds/marvelous.mp3",
    unbelievable: "sounds/unbelievable.mp3",
    select: "sounds/select.mp3",
    move: "sounds/move.mp3",
    disabledmove: "sounds/disabledmove.mp3",
    first:"sounds/first_of_many.mp3",
    indubitably:"sounds/indubitably.mp3",
    ...
    $element.on("click", function () {
        ...
        if ($gem1.length > 0) {
            ...
            if (Math.abs(gem1Row - gem2Row) <= 1 && Math.abs(gem1Col - gem2Col) <= 1) {
                ...
                //Get Gem2 position
                var swapTop1 = gem2Row * (boxSize + 1);
                var swapLeft1 = gem2Col * (boxSize + 1);

                // Animate Gem1 to Gem2 position
                $gem1.animate({left: swapLeft1 + "px", top: swapTop1 + 'px'}, 300);

                //Get Gem1 position
                var swapTop2 = gem1Row * (boxSize + 1);
                var swapLeft2 = gem1Col * (boxSize + 1);
...
function setSize(){
    boxSize = (window.innerWidth - 6)/7
    $('.inner-container').css({paddingTop: boxSize*3 + 'px'})
    $('.block-container').css({height: window.innerWidth + 'px'})
...
function generateMap() {
    ...
    for (var row = 0; row < rows; row++) {
        ...
        for (var col = 0; col < rows; col++) {
            ...
            var gem = {
                "image": "img/gem_" + randomNumber + ".png",
                "block": block,
...
function moveGemsDown(gemsMatch) {
    ...
    for (var col = 0; col < rows; col++) {
        ...
        for (var row = mapData.length - 1; row >= 0; row--) {
            _.each(emptyGemsOnCol, function (emptyGem, index) {
                ...
                if (currentGem.row < emptyGem.row) {
                    ...
                    var gemOffsetTop = rowOffset * (boxSize + 1);
                    ...
                    matchedGem.image = "img/gem_" + currentGem.gemType + ".png";
        ...
        _.each(emptyGemsOnCol, function (emptyGem, index) {
            ...
            $img.attr('src', "img/gem_" + randomType + ".png");

            $newDiv.find('.helper').append($img);
            $newDiv.css({'top': (-1 - index) * (boxSize + 1), 'left': col * (boxSize + 1), 'width':boxSize + 'px', 'height':boxSize + 'px'});

            $('#gem-container').append($newDiv);
            mapData[currentRow][col].gemType = randomType;
            mapData[currentRow][col].image = "img/gem_" + mapData[currentRow][col].gemType + ".png",

                $newDiv.animate({top: (emptyGemsOnCol.length - 1 - index) * (boxSize + 1) + 'px'}, 500, function () {
...
function renderGems(mapData) {
    ...
    for (var row = 0; row < mapData.length; row++) {
        for (var col = 0; col < mapData[row].length; col++) {
            ...
            $newDiv.find('.helper').append($img)
            $newDiv.css({'top': row * (boxSize + 1), 'left': col * (boxSize + 1), 'width':boxSize + 'px','height':boxSize + 'px'});
...
function destroyGems(gemsArray) {
    ...
    for (var i = 0; i < gemsArray.length; i++) {
        ...
        $gem.find('img').hide();
        $destroyAnimation.css({'width':boxSize+'px','height':boxSize + 'px'})
